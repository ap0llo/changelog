...
namespace Grynwald.ChangeLog.Templates.GitLabRelease
{
    ...
    internal class GitLabReleaseTemplate : MarkdownBaseTemplate
    {
        ...
        protected override MdBlock GetEntryHeaderBlock(ChangeLogEntryViewModel entry)
        {
            ...
            return new MdHeading(5, GetEntryTitle(entry));
        }

        /// <inheritdoc />
        protected override string GetHtmlHeadingId(ChangeLogEntryViewModel entry)
        {
            var summaryText = GetEntryTitle(entry);
            // use default header ids generated by GitLab instead of setting a header explicitly
            return new MdHeading(1, summaryText).Anchor ?? "";
        }
    }
}
