...
namespace Grynwald.ChangeLog.Test.Templates.Markdown
{
    /// <summary>
    /// Tests for <see cref="DefaultTemplate"/>
    /// </summary>
    public class MarkdownTemplateTest : TemplateTest
    {
        protected override ITemplate GetTemplateInstance(ChangeLogConfiguration configuration) => new MarkdownTemplate(configuration);
        ...
        public void When_normalization_is_disabled_default_text_is_rendered()
        {
            ...
            var testData = new TestDataFactory();
            var configuration = new ChangeLogConfiguration();
            configuration.Template.Markdown.NormalizeReferences = false;
