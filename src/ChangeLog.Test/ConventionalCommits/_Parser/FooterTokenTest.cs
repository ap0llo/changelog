...
{
    /// <summary>
    /// Tests for <see cref="FooterToken"/>
    /// </summary>
    public class FooterTokenTest
    {
        [Theory]
        [InlineData(0)]
        [InlineData(-5)]
        ...
        {
            Assert.Throws<ArgumentException>(() => new FooterToken(FooterTokenKind.String, "Some Value", lineNumber, 2));
        }
        ...
        {
            Assert.Throws<ArgumentException>(() => new FooterToken(FooterTokenKind.String, "Some Value", 1, columnNumber));
        }

        [Theory]
        [CombinatorialData]
        public void Two_token_instances_are_equal_if_all_properties_are_equal(
            FooterTokenKind kind,
        ...
        {
            var token1 = new FooterToken(kind, value, lineNumber, columnNumber);
            var token2 = new FooterToken(kind, value, lineNumber, columnNumber);
        ...
        [Fact]
        public void Equals_returns_false_if_the_argument_if_not_a_FooterToken()
        {
            var sut = new FooterToken(FooterTokenKind.String, "Some Value", 1, 2);
