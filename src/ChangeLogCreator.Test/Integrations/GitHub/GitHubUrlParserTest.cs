...
namespace ChangeLogCreator.Test.Integrations.GitHub
{
    public class GitHubUrlParserTest
    {
        ...
        {
            Assert.ThrowsAny<ArgumentException>(() => GitHubUrlParser.ParseRemoreUrl(url));
        }
        ...
        public void ParseRemoreUrl_returns_the_expected_GitHubProjectInfo(string url, string host, string owner, string repository)
        {
            // ARRANGE

            // ACT 
            var projectInfo = GitHubUrlParser.ParseRemoreUrl(url);
